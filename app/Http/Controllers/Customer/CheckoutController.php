...
class CheckoutController extends Controller
{
    ...
    public function review()
    {
        ...
        if (isset($sessionData['addresses']['delivery_address_id'])) {
            ...
            // Set default delivery fee if coordinates are not available
            $distance_in_miles = 0;
            $delivery_fee = 0;

            // Only calculate distance if both addresses have valid coordinates
            if ($this->companyAddress->latitude && $this->companyAddress->longitude && $delivery_address->latitude && $delivery_address->longitude) {
                ...
                // Use a standard delivery fee if coordinates are not available
                $delivery_fee = $this->price_per_mile;
            }
    ...
    public function proccessCheckout(Request $request)
    {
        $validated = $request->validate([
            'payment_method' => ['required', Rule::in(['cash_on_delivery', 'bkash'])],
            'transaction_id' => ['nullable', 'string', 'max:100', 'required_if:payment_method,bkash'],
        ...
        $order = Order::updateOrCreate(
            ...
            [
                ...
                'payment_status'     => $validated['payment_method'] === 'bkash' ? 'pending' : 'unpaid',
                'transaction_id'     => $validated['payment_method'] === 'bkash' ? $validated['transaction_id'] : null,
                'session_id'         => null,
        ...
        return $this->processLocalPayment($order, $user);
    ...
    private function processLocalPayment($order, $user)
    {
        ...
        try {
            \Mail::to($user->email)->send(new \App\Mail\CustomerBookingConfirmation($order));
        } catch (\Exception $e) {

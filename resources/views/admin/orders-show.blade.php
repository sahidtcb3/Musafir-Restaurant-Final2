...
<div class="main-panel">
    <div class="content-wrapper">
      ...
      @include('partials.order-stats')
        ...
        @if(($order->payment_status ?? $order->status_online_pay) !== 'paid')
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                @if ($order->status === 'pending' && $loggedInUser->role === 'global_admin')
                    <form action="{{ route('admin.orders.confirm', $order->id) }}" method="POST" class="d-inline">@csrf<button class="btn btn-success btn-sm">Confirm Order</button></form>
                    <button class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#rejectModal">Reject Order</button>
                @endif
                    ...
                    <div class="col-md-6">
                        <table class="table table-bordered mt-2">     
                            ...
                            <tr>
                                ...
                                <td>{{ ucfirst($order->payment_status ?? $order->status_online_pay ?? 'unpaid') }}</td>
                            </tr>
    ...
    @endif
        ...
        <div class="card mt-3">
            <div class="card-body">
                <form action="{{ route('admin.orders.payment-status', $order->id) }}" method="POST" class="row g-2 align-items-end">
                    @csrf
                    <div class="col-md-4"><label class="form-label">Payment Status</label><select name="payment_status" class="form-control">
                        @foreach(['pending', 'paid', 'unpaid', 'failed', 'refunded'] as $paymentStatus)
                            <option value="{{ $paymentStatus }}" {{ ($order->payment_status ?? $order->status_online_pay) === $paymentStatus ? 'selected' : '' }}>{{ ucfirst($paymentStatus) }}</option>
                        @endforeach
                    </select></div>
                    <div class="col-md-3"><button class="btn btn-primary">Update Payment</button></div>
                </form>
            </div>
        </div>
        @endif
                        ...
                        <form action="{{ route('admin.orders.update', $order->id) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="orderStatus" name="status">
                                    @foreach(['pending', 'confirmed', 'preparing', 'ready', 'out_for_delivery', 'delivered', 'cancelled', 'rejected'] as $orderStatus)
                                        <option value="{{ $orderStatus }}" {{ $order->status === $orderStatus ? 'selected' : '' }}>{{ ucwords(str_replace('_', ' ', $orderStatus)) }}</option>
                                    @endforeach
                                </select>
                            </div>
        ...
        <div class="modal fade" id="rejectModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog"><div class="modal-content">
                <div class="modal-header"><h5 class="modal-title">Reject Order</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <form action="{{ route('admin.orders.reject', $order->id) }}" method="POST">@csrf
                    <div class="modal-body"><label for="detail_rejection_reason" class="form-label">Reason (optional)</label><textarea class="form-control" id="detail_rejection_reason" name="rejection_reason" maxlength="1000"></textarea></div>
                    <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button type="submit" class="btn btn-danger">Reject Order</button></div>
                </form>
            </div></div>
        </div>

...
@section('content')
        ...
        <div class="container">
            ...
            @csrf  
                <div class="row justify-content-center">
                    ...
                    <div class="row mb-3">
                        <div class="form-group mb-3 mt-2 col-md-12">
                            <div class="heading_s1">
                                <h4>Payment Method</h4>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="payment_method" id="cash_on_delivery" value="cash_on_delivery" {{ old('payment_method', 'cash_on_delivery') === 'cash_on_delivery' ? 'checked' : '' }}>
                            ...
                            <div id="bkash-details" class="mt-3 p-3 border rounded" style="display: none;">
                                <p class="mb-2">Send payment to: <strong>{{ config('payments.bkash.number') ?: 'Contact the restaurant for the bKash number.' }}</strong></p>
                                <label for="transaction_id" class="form-label">bKash Transaction ID</label>
                                <input type="text" class="form-control" id="transaction_id" name="transaction_id" value="{{ old('transaction_id') }}" maxlength="100">
                                @error('transaction_id') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const details = document.getElementById('bkash-details');
            const transaction = document.getElementById('transaction_id');
            const updatePaymentDetails = function () {
                const isBkash = document.getElementById('bkash').checked;
                details.style.display = isBkash ? 'block' : 'none';
                transaction.required = isBkash;
            };
            radios.forEach((radio) => radio.addEventListener('change', updatePaymentDetails));
